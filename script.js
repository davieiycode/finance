...
function rebuildMetadataFromTransactions(txs, mode = 'merge') {
  ...
  newAccounts.forEach(a => accSet.add(String(a.name || '').toLowerCase()));
  newMerchants.forEach(m => merSet.add(String(m.name || '').toLowerCase()));
  newAuthors.forEach(a => autSet.add(String(a.name || '').toLowerCase()));
  newItems.forEach(i => itemSet.add(String(i.name || '').toLowerCase()));
...
window.onload = () => {
  ...
  typeButtons.forEach(btn => {
    btn.onclick = () => {
      ...
      const pGroup = document.getElementById('payment-account-group');
      const rGroup = document.getElementById('received-account-group');
      const type = String(btn.dataset.type || '').toLowerCase();
...
window.validateItemName = () => {
  ...
  if (val.length > 2) {
    const fuzzy = catalog.find(it => (String(it.name || '').toLowerCase().includes(String(val || '').toLowerCase())) || (String(it.sku || '').toLowerCase().includes(String(val || '').toLowerCase())));
    if (fuzzy && String(fuzzy.name || '').toLowerCase() !== String(val || '').toLowerCase()) {
...
function autoFillFromItem(item) {
  ...
  if (categoryInput && item.category) {
    ...
    if (catGroupInput) {
      ...
      const cat = categories.find(c => String(c.name || '').toLowerCase() === String(item.category || '').toLowerCase());
